...
magic_lookup = array('L', [ 0x00000000, 0x77073096, 0xEE0E612C,
...
class MagicWords:
	def __init__(self):
		self.word_1 = long(0x12e07897)
		self.word_2 = long(0x2f2dbb3d)
		self.word_3 = long(0x1bbdc9ae)
...
class Decryptor:
	...
	def decrypt(self, current_working_char):

		working_num = self.partial_decode_byte(current_working_char,
			self.magic_words.word_3)

		self.update_magic_words(working_num)
		return working_num
	


	def update_magic_words(self, partial_decode_byte):
		working_num1 = long(0)
		working_num2 = long(0)
		working_num3 = long(0)

		working_num1, working_num2 = self.part1(partial_decode_byte)
		working_num2 = self.part2(partial_decode_byte, working_num2)
		self.part3(partial_decode_byte, working_num1, working_num2)

	def part1(self, partial_decode_byte):
		working_num1 = long(0)
		working_num2 = long(0)

		working_num1 = partial_decode_byte ^ self.magic_words.word_1
		working_num2 = self.magic_words.word_1 >> 8
		working_num2 &= 0x00ffffff
		working_num1 &= 0xff
		working_num1 = magic_lookup[working_num1]
		working_num1 ^= working_num2
		working_num2 = self.al(working_num1)
		working_num2 += self.magic_words.word_2
		self.magic_words.word_1 = working_num1

		return (working_num1 &0xffffffff, working_num2 &0xffffffff)

	def part2(self, partial_decode_byte, working_num2):
		working_num2 *= 0x8088405
		working_num2 += 1
		self.magic_words.word_2 = working_num2

		return working_num2 &0xffffffff

	def part3(self, partial_decode_byte, working_num1, working_num2):
		working_num3 = long(0)

		working_num2 = working_num2 >> 0x18
		working_num2 &= 0x000000ff
		working_num2 ^= self.magic_words.word_3
		working_num2 &= 0xff
		working_num1 = self.magic_words.word_3
		working_num1 = working_num1 >> 8
		working_num1 &= 0x00ffffff
		working_num3 = magic_lookup[working_num2]
		working_num1 ^= working_num3
		self.magic_words.word_3 = working_num1 &0xffffffff
	

	def partial_decode_byte(self, encoded_byte, magic_word_3):
		working_num = 0
		working_num2 = 0
		working_num3 = 0
		...
		working_num3 = self.ah(working_num)
		working_num &= 0xffffff00
		working_num |= working_num3
	
		encoded_byte ^= self.al(working_num)
